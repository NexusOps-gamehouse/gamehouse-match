...
#     docker build -f match/Dockerfile -t gamehouse:match .
#
# 이유: 이 모듈은 :common 을 의존한다. 컨텍스트를 match/ 으로 잡으면
...
COPY gradle ./gradle
# settings.gradle 이 모듈 6개를 전부 선언하므로, 빌드 대상이 하나여도
# Gradle 은 설정 단계에서 6개 프로젝트를 모두 읽는다.
...
#
# build.gradle 은 몇 줄짜리라 6개를 다 복사해도 레이어가 거의 안 커지고,
...
RUN ./gradlew --no-daemon :match:dependencies

COPY common/src ./common/src
COPY match/src ./match/src
RUN ./gradlew --no-daemon :match:bootJar
RUN cp match/build/libs/*.jar /app/app.jar && mkdir -p /out/uploads
...
# 볼륨이 소유권을 상속받아 쓰기 가능하도록 spring 소유로 준비 (match는 지금 업로드 파일이 없지만
# 나머지 서비스와 이미지 구조를 맞춰 둔다)
