#!/usr/bin/env bash
#
# 로컬 DB 에 duo_match 계정과 match_svc 스키마를 만든다.
#
...
SVC_PASS=$(secret_value MATCH_DB_PASSWORD)

: "${ADMIN_USER:=duo}"
[ -n "$SVC_PASS" ] || { echo "application-secret.yml 에 MATCH_DB_PASSWORD 가 없습니다." >&2; exit 1; }
